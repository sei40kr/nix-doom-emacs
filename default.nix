...
let
  ...
  # Note: doomLocal already runs 'doom install' via nix-straight, which generates
  # the profile files in $DOOMLOCALDIR/etc/@/init.30.2.el
  # Doom 3.0 expects profiles at: etc/{profile}/@/{revision}/init.{version}.el
  straight-emacs-env = runCommand "straight-emacs-env" {} ''
    # Copy everything from doomLocal (includes packages + profile files)
    mkdir -p "$out"
    cp -r ${doomLocal}/* "$out/"

    # Make copied files writable so we can restructure them
    chmod -R u+w "$out"

    # Verify profile files were generated by doomLocal
    if [[ ! -f "$out/etc/@/init.30.2.el" ]]; then
      echo "ERROR: Profile init file not found at $out/etc/@/init.30.2.el"
      echo "This should have been generated by doomLocal during 'doom install'"
      exit 1
    fi

    # Doom 3.0 expects profile at: etc/@/@/0/init.30.2.el
    # But doomLocal generates it at: etc/@/init.30.2.el
    # Restructure the profile directory to match Doom's expectations
    mkdir -p "$out/etc/@/@/0"
    mv "$out/etc/@/init.30.2.el" "$out/etc/@/@/0/"
    mv "$out/etc/@/init.30.d" "$out/etc/@/@/0/" 2>/dev/null || true

    echo "Profile files restructured successfully:"
    ls -la "$out/etc/@/@/0/" || true
  '';
  ...
  profile-loader = writeTextDir "share/doom/profiles.30.el" ''
    ...
    (pcase (intern (getenv-internal "DOOMPROFILE"))
      ('@ ;; Default profile (@ is the default profile name in Doom 3.0)
       ;; Profile files are at: $DOOMLOCALDIR/etc/@/@/0/init.30.2.el
       (load (file-name-concat (getenv "DOOMLOCALDIR") "etc/@/@/0/init.30.2.el") nil t)
