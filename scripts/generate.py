...
def generate_recipe_expr(name: str, files: JsonValue, repo_url: str | None) -> str:
    ...
    files_spec = format_files_spec(files)
    ...
    recipe_content = f'({name} :fetcher {fetcher}{repo_spec} :files {files_spec})'
...
def generate_package_expr(pkg: PackageSpec, pin_lookup: dict[str, str]) -> str:
    """Generate Nix expression for a single package."""
    name = pkg.name
    pin = pkg.pin or pin_lookup.get(name)
    repo_url = pkg.repo
    files = pkg.files
    ...
    if not pin:
        return f"  # {name}: No pin available"
    ...
    hash_value, melpa_version = prefetch_git_hash(repo_url, pin)
    ...
    fetcher_expr = generate_fetcher_expr(
        fetcher, owner, repo, pin, repo_url, domain, hash_value
    )

    # elpaBuild doesn't need recipe, melpaBuild does
    if builder == "elpaBuild":
        recipe_section = ""
    else:
    ...
    if melpa_version:
        ...
        version = f"unstable-{pin[:7]}" if pin else "unstable"
...
def build_pin_lookup(packages: list[PackageSpec]) -> dict[str, str]:
    """Build a lookup table for pins by package name.

    Some packages reference the same repo with null pins, so we need
    to find the pin from another package with the same repo.
    """
    repo_to_pin: dict[str, str] = {}
    pin_lookup: dict[str, str] = {}

    # First pass: collect repo -> pin mappings
    for pkg in packages:
        if pkg.repo and pkg.pin:
            repo_to_pin[pkg.repo] = pkg.pin

    # Second pass: resolve null pins
    for pkg in packages:
        if pkg.pin:
            pin_lookup[pkg.name] = pkg.pin
        elif pkg.repo and pkg.repo in repo_to_pin:
            pin_lookup[pkg.name] = repo_to_pin[pkg.repo]
...
def main() -> None:
    ...
    pin_lookup = build_pin_lookup(packages)
    ...
    for i, pkg in enumerate(packages):
        ...
        expr = generate_package_expr(pkg, pin_lookup)
        package_exprs.append(expr)

    # Write output
    output = f"""# Generated by scripts/generate.py - DO NOT EDIT MANUALLY
{{ lib, fetchFromGitHub, fetchFromGitLab, fetchFromGitea, fetchgit, melpaBuild, elpaBuild, writeText }}:
